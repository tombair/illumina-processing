...
def checkDirectory (dir):
    if os.path.exists(dir+"/Basecalling_Netcopy_complete.txt") == False:
...
def runSampleSheet (dir,size,sampleSheet):
    ...
    if size > 1:
        bclOut = subprocess.check_output(['/opt/illumina/bin/configureBclToFastq.pl', '--sample-sheet', sampleSheet, '--input-dir', dir+'/Data/Intensities/BaseCalls','--output-dir', output , '--ignore-missing-bcl','--ignore-missing-stat','--fastq-cluster-count=0',"--use-bases-mask=y*,I%sN*,y*" % (size,)])
    else:
        bclOut = subprocess.check_output(['/opt/illumina/bin/configureBclToFastq.pl', '--sample-sheet', sampleSheet, '--input-dir', dir+'/Data/Intensities/BaseCalls','--output-dir', output , '--ignore-missing-bcl','--ignore-missing-stat','--fastq-cluster-count=0'])
    logger.info(bclOut)
    return output
    ...
    if csv != None:
        if config.get('validate_waiting','locked') == 'False':
            ...
            for s in [0,6,8]: #sizes of indexes
                ...
                if csvFound != None:
                    logger.info("Found good sample sheet for size % running bclToFastQ for %s in %s" %(s,csvFound,dir))
